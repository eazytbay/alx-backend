...
def get_user():
    """Gives information about a user as a dictionary containing
    mock user data
    """
...
@app.before_request
def before_request():
    """This creates a global 'user' variable if the 'login_as' parameter was passed
    """
    user = get_user()
    g.user = user
...
def get_locale():
    """Functions that returns the locale requested based on several priorities"""
    ...
    header_req = request.accept_languages.best_match(app.config["LANGUAGES"])
    if header_req is not None:
        print("Req - Header3")
        return header_req
